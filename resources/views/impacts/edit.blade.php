...
<div class="col-lg-12">>
    <div class="card">
        <div class="card-header">
            <h4 class="card-title">Edit Impact</h4>
        </div>
        <div class="card-header">
            <a class="btn btn-primary" href="{{ route('index.impacts') }}" enctype="multipart/form-data"> Back</a>
        </div>
        @if(session('status'))
        ...
        @endif
        <div class="card-body">
            <form action="{{ route('update.impacts', $impact->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-group">
                    <label for="title" class="form-label">Title:</label>
                    <input type="text" name="title" class="form-control" placeholder=""
                        value="{{ old('title', $impact->title) }}">
                    @error('title')
                ...
                <div class="form-group">
                    ...
                    <textarea name="desc" rows="4" cols="30" class="form-control tinymce-editor"
                        required>{{ old('desc', $impact->desc) }}</textarea>
                    @error('desc')
                    <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="location" class="form-control" placeholder=""
                        value="{{ old('location', $impact->location) }}">
                    @error('location')
                    <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <button type="submit" class="btn btn-primary mt-4 mb-0">Update Impact</button>
            </form>
        </div>
    </div>
</div>
